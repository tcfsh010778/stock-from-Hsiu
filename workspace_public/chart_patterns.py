...
def annotate(bars):
    ...
    def observation(pid,start,evidence,confirmed=None):
        observations.append({'id':pid,'name':PATTERNS[pid],'start':b[start]['time'],'end':b[-1]['time'],
            ...
            'evidence':evidence,'basis':'engineering_geometry_v2','role':'pattern_filter_only'})
    ...
    if len(b)>=20:
        ...
        if len(lows)>=2 and b[lows[-1]]['low']>b[lows[-2]]['low']*1.01:
            observation('higher_lows',lows[-2],'兩個已確認低點上移 >1%；各需右側 3 根確認',lows[-1]+3)
        ...
        if len(lows)>=2 and len(highs)>=2:
            ...
            if inside and hv(end)>lv(end):
                ...
                if ls>0 and abs(b[h2]['high']/b[h1]['high']-1)<=.02:pids.append('ascending_triangle')
                if hs<0 and abs(b[l2]['low']/b[l1]['low']-1)<=.02:pids.append('descending_triangle')
        ...
        for inverted,pivots,opposites,field in [(False,highs,lows,'high'),(True,lows,highs,'low')]:
            ...
            if head and abs(values[0]/values[2]-1)<=.04 and between1 and between2:
                ...
                broken=b[-1]['close']>neck if inverted else b[-1]['close']<neck
                observation(pid,a,('末根收盤已越過頸線' if broken else '尚未突破頸線，僅為候選')+'；肩差 ≤4%，頭突出 ≥3%，左右各 3 根確認')
                line(pid,[(i,b[i][field if i in [a,h,c] else f]) for i in [a,n1,h,n2,c]])
                line(pid,[(n1,b[n1][f]),(len(b)-1,neck)])
    return {'version':'chart-annotations-v2','events':events,'lines':lines,'observations':observations,
