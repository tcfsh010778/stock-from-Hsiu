...
| `daily_market_flow` | Derived aggregate of TWSE T86 and TPEx three-institution OpenAPI | Each trading day after both official feeds | Max 2 calendar-day lag; source dates remain visible | None; source partitions visible | Publishes listed/OTC foreign and investment-trust buy/sell totals plus top net-buy/sell summaries; its derived artifact uses calendar-day freshness because it does not own a second trading-calendar route |
...
| `weekly_holder_risers` | Derived from the latest two weekly `holding_shares` snapshots | Weekly after holder refresh | Max 10 calendar days | None; cache state visible | Lists positive changes in the 400+ lots major-holder ratio; this is an observation filter, not a buy signal |
