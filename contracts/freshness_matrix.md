...
| `daily_market_flow` | TWSE BFI82U / TPEx amount summary, TWSE T86 / TPEx institutional detail and 20-session history, TWSE/TPEx margin balances, weekly TDCC holder distribution, and official recent price volume | Each trading day after the aligned official daily feeds; TDCC comparison advances weekly | Max 2 calendar-day lag for daily feeds; latest complete TDCC pair is retained | None; fail closed when a required source partition or the 20-session institutional window is unavailable | Homepage publishes exact official monetary totals; institutional Top 50 shows 5/10/20-session net shares and signed 20-session institutional concentration ratio, while holder Top 50 owns the weekly retail, margin-change, and short-to-margin fields |
...
| `refresh_prices.py` | Official TWSE/TPEx full-market OHLCV with exact-date historical backfill; optional legacy FinMind auxiliary caches are disabled unless explicitly enabled | Writes `price_refresh_summary.json`; aligned latest snapshots are preferred, while a latest-date skew may recover only from same-day TWSE/TPEx historical partitions with no duplicate security IDs, absolute unique-ID floors, and at least 99% coverage of each latest-snapshot reference ID set; recovery evidence is recorded in `latest_snapshot`; zero matched/written files or incomplete partitions still fail closed; chip/holding/foreign-shareholding/margin migration remains separate |
