...
<meta name="color-scheme" content="dark"><title>Stock from Hsiu V2</title>
...
<main class="shell">
  ...
  <div id="app" hidden>
    ...
    <section id="overview" class="pane card active">
      ...
      <section id="analysis-entry" class="analysis-entry">
        <div class="section-head"><div><div class="eyebrow">ANALYSIS ENTRY</div><h2>V2 分析入口 <span class="tag">平行路徑</span></h2></div></div>
        <p class="notice">這裡集中目前可重現的技術與籌碼證據；不取代 V1，也不新增未驗證的規則門檻。</p>
        <div class="analysis-entry-grid">
          <button class="analysis-entry-link" data-pane="technical-evidence"><strong>技術分析證據卡</strong><span class="entry-status">K 線、線型、量能、RSI、MACD、布林</span></button>
          <button class="analysis-entry-link" data-pane="chips"><strong>籌碼分析</strong><span class="entry-status">法人、外資持股與股東結構分開呈現</span></button>
          <button class="analysis-entry-link" data-pane="plan"><strong>現行策略證據</strong><span class="entry-status">沿用既有資料，不新增指標門檻</span></button>
          <button class="analysis-entry-link" data-pane="history"><strong>歷史分析</strong><span class="entry-status">報告／回測統一入口骨架，後續整理</span></button>
        </div>
      </section>
      <div class="method-note"><b>固定規則：</b>Python 只計算型態、趨勢線、支撐壓力與 15% 停損價；畫面不產生買賣指令，也不替使用者決定方向。</div>
    </section>

    <section id="technical-evidence" class="pane card">
      <div class="section-head"><div><div class="eyebrow">TECHNICAL EVIDENCE</div><h2>技術分析證據卡</h2></div><span class="tag">日線／已閉合 K 線</span></div>
      <p class="notice">RSI、MACD、布林軌道與量能比較是輔助證據；數值不足時保留缺口，不單獨產生訊號。</p>
      <div id="technical-evidence-cards" class="evidence-grid"></div>
      <h3>目前 K 線與線型判讀</h3>
      <div id="technical-pattern-summary" class="grid"></div>
    </section>
    ...
    <section id="chips" class="pane card">
      <div class="section-head"><div><div class="eyebrow">B · CHIP EVIDENCE</div><h2>K 線與籌碼工作台</h2></div><div id="crosshair-date" class="crosshair-date">日 K 是主視窗；籌碼副圖各自縮放</div></div>
      <div id="chip-status"></div>
      <div id="annotation-detail" class="method-note">K 線型態只描述已收盤日 K 的形狀與相對位置，不代表交易訊號。</div>
    ...
    <section id="history" class="pane card">
      <div class="section-head"><div><div class="eyebrow">HISTORICAL ANALYSIS</div><h2>歷史報告與回測入口</h2></div><span class="tag">整理中</span></div>
      <p class="notice">此處先保留共同分析入口；歷史報告與回測資料會在契約與驗證補齊後再接入。</p>
    </section>
  </div>
</main>
...
function renderHeader(meta){
  ...
  $("#meta").innerHTML=`<span>${d.price_adjustment?.verified?"經事件核對的還原價":"價格口徑未驗證"}</span><span>資料 ${esc(d.data_date)}</span><span>引擎 ${esc(d.engine_version)}</span><span>品質 ${esc(d.freshness?.status||"unknown")}</span>`;
...
function renderChipStatus(){
  ...
  $("#chip-status").innerHTML=`<div class="gap-list">${available.map(k=>`<span class="available">${gapNames[k]} · ${m.source_status?.[k]?.state==="stale"?"舊資料":"最新"} · ${esc(m.source_dates?.[k]||"")}</span>`).join("")}${gaps.map(k=>`<span class="gap">缺少 ${gapNames[k]}</span>`).join("")}</div>`;
}
...
function addEntry(el,rows,primary,key){const chart=primary.chart,entry={chart,primary:primary.series,rows,lookup:new Map(rows.map(r=>[r.date,r])),value:r=>finite(r[key])?Number(r[key]):null};chartEntries.push(entry);return entry}
...
def stock_redirect_html(stock_id: str) -> str:
    target = f"../stock.html?id={stock_id}"
    return (
...
V2_CSS += "#ma-status{line-height:1.9}"

V2_CSS += '.latest-btn{border:1px solid var(--line);background:#172235;color:#cbd6e5;border-radius:4px;padding:3px 9px;cursor:pointer}.latest-btn:hover,.latest-btn:focus{border-color:var(--cyan);color:white}.annotation-note{color:#b8c7da}.annotation-note b{color:#e8eef8}'
