...
V2_CSS += r'''.latest-btn{border:1px solid var(--line);background:#172235;color:#cbd6e5;border-radius:4px;padding:3px 9px;cursor:pointer}.latest-btn:hover,.latest-btn:focus{border-color:var(--cyan);color:white}'''
...
function initChipWorkbench(){
  ...
  const priceEl=panel("日 K 與成交量","主視窗 · 紅漲綠跌 · 縮放保持最新端",430,true),priceChart=L.createChart(priceEl,baseOptions());
  ...
  priceEl.closest(".tv-panel").querySelector(".latest-btn").onclick=()=>priceChart.timeScale().scrollToRealTime();
  priceEl.addEventListener("wheel",()=>{const pinLatest=priceChart.timeScale().scrollPosition()<=1;if(pinLatest)setTimeout(()=>priceChart.timeScale().scrollToPosition(0,false),0)},{capture:true,passive:true});
