...
function conditionNumbers(m){
 const names={close:'收盤',ma240:'MA240',previous_ma240:'前日 MA240',bars:'歷史根數',required_bars:'所需根數',one_year_high:'曾創一年新高',A:'MA240 上揚',deduction_close:'240 日扣抵價',rebound_high:'反彈高點',range_20:'近 20 根振幅',preceding_range_60:'此前 60 根振幅',range_midpoint_80:'80 根區間中點',prior_low_20:'此前 20 根最低價',recent_low_5:'近 5 根最低價',higher_lows:'低點墊高',expanded_up_days_60:'60 根內放量上漲日數',expanded_down_days_20:'20 根內放量下跌日數',volume_at_60bar_minimum:'均量達區間最低',long_lower_shadows_5:'近 5 根長下影數',absolute_deviation_ma20_pct:'MA20 絕對乖離 %',delta_percentage_points:'較前週增減（百分點）',major_delta:'六週大戶增減（百分點）',retail_delta:'六週散戶增減（百分點）',major_delta_percentage_points:'大戶增減（百分點）',shareholder_delta:'股東人數增減',retail_lots:'散戶級距上限（張）',periods:'股權期數',start:'起日',end:'迄日',observed:'已有交易日期',required:'所需交易日期',change_lots:'持股變化（張）',price_change_pct:'股價變動 %',increase_sessions:'增加日數',mom_pct:'月增率 %',yoy_pct:'年增率 %',period:'營收月份',below_low:'已跌破低點'};
 const volume={volume_5:'近 5 根均量',preceding_volume_20:'此前 20 根均量',volume_ma5:'5 日均量',volume_ma20:'20 日均量',deduction_volume:'20 日扣抵量'};
 let out=Object.entries(m||{}).flatMap(([k,v])=>{const label=names[k]||volume[k];if(!label)return [];const value=v==null?'未知':typeof v==='boolean'?conditionStatus(v):typeof v==='number'?num(volume[k]?v/1000:v,2):String(v);return [`<div><dt>${esc(label)}${volume[k]?'（張）':''}</dt><dd>${esc(value)}</dd></div>`];}).join('');
 if(m?.pivots)out+=m.pivots.map(p=>`<div><dt>低點 ${esc(p.date)}</dt><dd>${num(p.price)} · ${esc(p.confirmed_at)} 確認</dd></div>`).join('');
 for(const [key,label] of [['foreign','外資持股'],['trust','投信持股']])if(m?.[key])out+=`<div><dt>${label}</dt><dd>${conditionStatus(m[key].decision)} · ${m[key].observed}/${m[key].required} 期 · ${num(m[key].change_lots,0)} 張</dd></div>`;
 if(m?.dates)out+=`<div><dt>資料日期</dt><dd>${esc(m.dates.join(' → '))}</dd></div>`;
 if(m?.industries)out+=`<div><dt>產業分類</dt><dd>${esc(m.industries.map(i=>i.name).join('、')||'未知')}</dd></div>`;
 return out?`<dl class="condition-values">${out}</dl>`:'<p class="muted">目前沒有足以證實此條件的數值。</p>';
}
