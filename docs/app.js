...
const cache = new Map();
let aiPatterns={};
function renderPatternOptions(){if(!index)return;const source=$('pattern-source').value,value=$('pattern').value,catalog=index.pattern_catalogs[source];$('pattern').innerHTML='<option value="">所有型態</option>'+Object.entries(catalog).map(([id,label])=>`<option value="${esc(id)}">${esc(label)}</option>`).join('');if(value in catalog)$('pattern').value=value;}
...
function renderList(){
 ...
 const stocks=index.stocks.filter(s=>{
  ...
  const condition=$('condition-filter').value;if(condition&&!(s.mda.matched_conditions||[]).includes(condition))return false;
  const ax=$('ax-filter').value;if(ax&&s.mda.familiar_pattern?.[ax]?.decision!==true)return false;
 ...
 const source=$('mda-source').value;stocks.sort((a,b)=>source==='外資連買至少3日'?b.mda.sources.foreign_consecutive.sessions-a.mda.sources.foreign_consecutive.sessions:source==='投信連買至少3日'?b.mda.sources.trust_consecutive.sessions-a.mda.sources.trust_consecutive.sessions:source==='六週股權趨勢'?b.mda.sources.holder_six_weeks.net_change-a.mda.sources.holder_six_weeks.net_change:0);
 $('result-count').textContent=`${stocks.length} 檔`;
 $('empty').hidden=stocks.length>0;
 $('stock-list').innerHTML=stocks.map(s=>`<tr data-stock="${esc(s.stock_id)}" tabindex="0" aria-label="開啟 ${esc(s.stock_id+' '+s.name)}" class="${selected===s.stock_id?'selected':''}"><td><strong>${esc(s.name)}</strong><small>${esc(s.stock_id)} · ${esc(s.market||'—')}</small></td><td>${num(s.close)}</td><td class="${color(s.change_pct)}">${pct(s.change_pct)}</td><td>${s.sfz.member?'<span class="badge good">SFZ 初篩</span>':''}${s.mda.member?'<span class="badge warn">M 大觀察</span>':''}${s.mda.member?'<small class="source-reasons">'+esc((s.mda.source_labels||[]).join(' · '))+'</small>':''}${!s.sfz.member&&!s.mda.member?'<span class="muted">未入選</span>':''}</td></tr>`).join('');
 if(!selected&&stocks.length)openStock(stocks[0].stock_id);
}
async function openStock(sid){
 const request=++generation;
 selected=sid;chartOffset=0;chartCount=Number($('chart-range').value)||120;for(const key of Object.keys(chartYZoom))delete chartYZoom[key];renderList();
 ...
 try{
  ...
  cache.set(sid,data);detail=data;if($('chart-range').value==='all')chartCount=data.candles[frame].length;
  ...
  $('candle-status').textContent=data.candlestick_calculation?.status==='ok'?`TA-Lib ${data.candlestick_calculation.catalog.pattern_function_count} 種型態可用；公開採已審查的中性日 K 註記。週／月 K 不套用日 K 訊號。`:'TA-Lib 註記尚未計算或資料不足；不補造型態。';
...
document.querySelectorAll('[data-frame]').forEach(button=>button.onclick=()=>{frame=button.dataset.frame;chartOffset=0;chartCount=$('chart-range').value==='all'?detail?.candles[frame].length||120:Number($('chart-range').value)||120;document.querySelectorAll('[data-frame]').forEach(b=>b.classList.toggle('active',b===button));renderChart();$('ai-result').textContent='週期已切換；請重新分析這張圖表。';});
...
$('pattern-source').onchange=()=>{renderPatternOptions();const useAI=$('pattern-source').value==='ai';$('ai-filter-status').hidden=!useAI;if(useAI)refreshAIPatterns();else renderList();};
$('chart-range').onchange=()=>{chartOffset=0;chartCount=$('chart-range').value==='all'?detail?.candles[frame].length||120:Number($('chart-range').value)||120;renderChart();};
...
window.addEventListener('DOMContentLoaded',()=>{getJSON('data/index.json').then(data=>{if(data.schema_version!=='sfz-mda-workspace-1')throw Error('資料版本不相容');index=data;$('data-date').textContent=data.data_date;$('coverage').textContent=`${data.coverage.verified.toLocaleString()} 檔有驗證歷史`;$('sfz-count').textContent=data.stocks.filter(s=>s.sfz.member).length;$('mda-count').textContent=data.stocks.filter(s=>s.mda.member).length;$('weekly-date').textContent=`股權資料 ${data.weekly_date||'待更新'}`;$('notification-status').textContent=data.services.telegram;$('freshness').textContent=data.fresh?'● 盤後資料已對齊':'◷ 保留最近可驗證資料';$('freshness').classList.add(data.fresh?'good':'warn');$('condition-filter').innerHTML='<option value="">不限</option>'+index.condition_catalog.map(c=>`<option value="${esc(c.id)}">${esc(c.id+' '+c.label)}</option>`).join('');
 renderPatternOptions();$('heat-chain').innerHTML+=[...new Set(data.stocks.flatMap(s=>(s.industry||[]).map(i=>i.chain)))].sort().map(c=>`<option>${esc(c)}</option>`).join('');renderHealth();renderMacro();renderHeatmap();const sid=new URL(location.href).searchParams.get('stock');if(sid&&data.stocks.some(s=>s.stock_id===sid))openStock(sid);else renderList();}).catch(error=>message(error.message));},{once:true});
