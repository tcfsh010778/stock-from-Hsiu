...
- Local public focused suite passed 61 tests including route redirection and year-boundary regression; private full CI run 34479008898 passed Linux and Windows (345 tests plus 14 subtests). Public CI run 34479025760 passed both platforms (40 unittest tests and 219 pytest tests plus 4 subtests). Read-only review findings in outbox unknown-state preservation, stale CDN verification and archive event provenance were fixed and rechecked.
- Actual cloud daily dry-run 34479020160 exposed non-JSON TWSE latest OpenAPI responses. Added an official historical fallback anchored only to the surviving market's verified date, retaining both-market minimum coverage and surviving identity checks; both endpoints unavailable stops without guessing a date or writing. All 13 adapter tests passed and read-only review found no blockers. The dry-run retry is 34479637579; no publish/send occurs in dry-run.
- Desktop and 390px mobile preview passed search, filters, chart period and no-horizontal-overflow checks. AI missing-key and stale-cache handling were checked without provider calls. Next: finish actual cloud producer qualification, then activate only after secure Telegram/OpenAI configuration and review of any paid hosting. No actual provider call, real Telegram send, live order or public deployment is claimed.
...
- Additional checks: committed price/sidecar blob bytes for all 1,459 pairs equal disk bytes and both declared SHA-256 values; no CRLF pollution. Source PR17 merged as 9758d6aeb9a8d7e70f6b69433baa101837c1012c; source-date PR18 merged as 7b7dea1825a5a7fe026068615210aebfa01eb349.
- Full local artifact, V2 navigation, review-home and holder-flow checks pass. End-to-end run 34387136066 passed all generators but exposed review-shell metadata missing from the legacy date gate; fixed with explicit build-session metadata. V2 verification now checks actual JSON-driven homepage links, including zero-alert baselines, rather than requiring nonexistent static cards. Strict search links and per-stock dates remain enforced.
- Final end-to-end dry run 34390742670 passed every generator and release gate. Public CI 34391298534 passed Ubuntu/Windows; PR37 merged as 47839809de429f77f796e262db5cf09926804532 with identical tested tree a11a65d6a661ac6d5ebe936fcf1fbd3b0d4908f5. Live Pages acceptance and final release SHA are recorded in Issue34.
- Live publication exposed GitHub returning the numeric /repositories/1225747295/pages/builds/latest API alias. The verifier now recognizes only that trusted API shape and polls the configured owner/repo endpoint, preserving exact-commit and CDN-byte checks. Nine focused tests pass, including wrong-commit and untrusted-alias rejection. No generated data changed in this follow-up.
